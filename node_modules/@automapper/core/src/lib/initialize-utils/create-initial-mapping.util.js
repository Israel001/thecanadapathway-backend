...
function createInitialMapping(sourceObj, destinationObj, sourceNestedMetadataMap, destinationNestedMetadataMap, saveMapping, options, createInitialMappingOptions) {
    ...
    const mapping = [
        ...
        [],
        undefined,
        get_naming_conventions_from_options_util_1.getNamingConventionsFromOptions(conventions),
        undefined,
    ];
    if (prePropertiesLoop) {
        prePropertiesLoop(mapping);
    }
    const destinationPaths = get_path_recursive_util_1.getPathRecursive(destinationObj) || [];
    ...
    for (let i = 0, destinationPathsLen = destinationPaths.length; i < destinationPathsLen; i++) {
        ...
        const destinationNestedMetadataAtPath = get_nested_metakey_at_destination_path_util_1.getNestedMetaKeyAtDestinationPath(destinationNestedMetadataMap, sourceNestedMetadataMap, destinationPath, namingConventions);
        const sourcePath = get_source_property_path_util_1.getSourcePropertyPath(destinationPath, namingConventions);
        ...
        if (!isDerivedSourcePathOnSource(sourceObj, sourcePath) &&
            utils_1.isDefined(namingConventions)) {
            const sourcePaths = get_flattening_source_paths_util_1.getFlatteningSourcePaths(sourceObj, sourcePath, namingConventions);
            if (!utils_1.isDefined(sourcePaths)) {
                continue;
            }
            mapping[2 /* properties */].push([
                destinationPath,
                [[destinationPath], [map_initialize_1.mapInitialize(sourcePaths), isMetadataNull]],
        ...
        mapping[2 /* properties */].push([
            ...
            [
                ...
                [map_initialize_1.mapInitialize(sourcePath), isMetadataNull],
            ],
            destinationNestedMetadataAtPath,
        ]);
    }
    ...
    if (bases.length) {
        extend_mappings_util_1.extendMappings(bases, mapping);
    }
    saveMapping(mapping);
    return mapping;
}
