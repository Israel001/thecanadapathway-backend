...
function getNestedMetaKeyAtDestinationPath(destinationNestedMeta, sourceNestedMeta, destinationPath, namingConventions) {
    ...
    if (!utils_1.isEmpty(sourceNestedMeta) && !utils_1.isEmpty(destinationNestedMeta)) {
        let sourceNestedMetaAtPath;
        const destinationNestedMetaAtPath = (_a = destinationNestedMeta.find(([destinationNestedMetaPath]) => utils_1.isSamePath(destinationNestedMetaPath, destinationPath))) === null || _a === void 0 ? void 0 : _a[1];
        if (utils_1.isDefined(namingConventions)) {
            const [sourceNamingConvention, destinationNamingConvention] = namingConventions;
            sourceNestedMetaAtPath = (_b = sourceNestedMeta.find(([sourceNestedMetaPath]) => utils_1.isSamePath(sourceNestedMetaPath.map((s) => destinationNamingConvention.transformPropertyName(s
                .split(sourceNamingConvention.splittingExpression)
                .filter(Boolean))), destinationPath))) === null || _b === void 0 ? void 0 : _b[1];
        }
        else {
            sourceNestedMetaAtPath = (_c = sourceNestedMeta.find(([sourceNestedMetaPath]) => utils_1.isSamePath(sourceNestedMetaPath, destinationPath))) === null || _c === void 0 ? void 0 : _c[1];
        }
