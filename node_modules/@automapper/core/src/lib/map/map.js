...
function setMemberMutateFn(destinationObj) {
    return (destinationMember) => (value) => {
        if (value !== undefined) {
            utils_1.setMutate(destinationObj, destinationMember, value);
        }
    };
}
function getMemberMutateFn(destinationObj) {
    return (memberPath) => utils_1.get(destinationObj, memberPath);
}
function setMemberReturnFn(destinationMemberPath, destination) {
    return (value) => {
        destination = utils_1.set(destination, destinationMemberPath, value);
    };
}
/**
 *
...
Original error: ${originalError}`;
        ...
        if (transformationMapFn[0 /* type */] ===
            ...
            if (typeof mapInitializedValue === 'object') {
                ...
                utils_1.mappingNullCheck(nestedMapping, errorHandler, nestedSourceMemberKey, nestedDestinationMemberKey);
                // nested mutate
        ...
        setMember(() => map_member_1.mapMember(transformationMapFn, sourceObj, destination, destinationMemberPath, extraArguments, mapper));
    }
    // After map
    ...
    assert_unmapped_properties_1.assertUnmappedProperties(destination, configuredKeys, sourceKey, destinationKey, errorHandler);
    return destination;
}
/**
 *
...
function mapArray(sourceArray, destination, source, options, mapper, errorHandler) {
    ...
    for (let i = 0, len = sourceArray.length; i < len; i++) {
        ...
        utils_1.mappingNullCheck(mapping, errorHandler, source, destination);
