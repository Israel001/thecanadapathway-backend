...
function getDecoratorOrUndefinedByNames(names, factory, decorators) {
    return (decorators || factory.createNodeArray()).find((item) => names.includes(ast_utils_1.getDecoratorName(item)));
}
exports.getDecoratorOrUndefinedByNames = getDecoratorOrUndefinedByNames;
function getTypeReferenceAsString(type, typeChecker) {
    if (ast_utils_1.isArrayType(type)) {
    ...
    if (ast_utils_1.isNumber(type) || ast_utils_1.isNumberEnum(type)) {
        return Number.name;
    }
    if (ast_utils_1.isString(type) || ast_utils_1.isStringEnum(type)) {
        return String.name;
    }
    if (ast_utils_1.isDate(type)) {
        return Date.name;
    }
    if (type.isClass()) {
