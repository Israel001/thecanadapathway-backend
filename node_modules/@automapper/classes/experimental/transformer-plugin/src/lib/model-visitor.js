...
class ModelVisitor {
    ...
    static visit(sourceFile, context, program) {
        ...
        function visitor(ctx, sf) {
            const _visitor = (node) => {
                if (tsserverlibrary_1.isImportDeclaration(node)) {
                    ModelVisitor.importsMap.set(node.moduleSpecifier.text, ModelVisitor.cloneImportDeclaration(ctx.factory, node));
                    return node;
                }
                if (tsserverlibrary_1.isClassDeclaration(node)) {
                    node = tsserverlibrary_1.visitEachChild(node, _visitor, ctx);
                    return ModelVisitor.addMetadataFactory(node, ctx.factory);
                }
                if (tsserverlibrary_1.isPropertyDeclaration(node) || tsserverlibrary_1.isGetAccessorDeclaration(node)) {
                    const decorators = node.decorators;
                    const existingAutoMapDecorator = plugin_utils_1.getDecoratorOrUndefinedByNames([constants_1.AUTOMAPPER_DECORATOR_NAME], ctx.factory, decorators);
                ...
                return tsserverlibrary_1.visitEachChild(node, _visitor, ctx);
            };
            return _visitor;
        }
        const _sourceFile = tsserverlibrary_1.visitNode(sourceFile, visitor(context, sourceFile));
    ...
    static createPropertyAssignment(node, typeChecker, existingProperties, hostFileName, factory) {
        ...
        if (!key || ast_utils_1.hasPropertyKey(key, existingProperties)) {
            return undefined;
        }
        ...
        if (ast_utils_1.isNullableUnionType(type)) {
    ...
    static shouldCreateNullNode(type, typeChecker, node) {
        return ast_utils_1.isTypeLiteral(type) || ast_utils_1.isAnyType(type, typeChecker, node);
    }
}
